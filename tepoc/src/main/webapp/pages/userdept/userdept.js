...
//						}
                 ...
                 *  treeset 树控件的配置obj
                 */
                showDeptTreeDiv_UserDept: function (sendjson, ajaxurl, treetitle, treeset) {
                    $.ajax({
                        type: "GET",
                        url: ajaxurl,
                        contentType: 'application/json;charset=utf-8',
                        ...
                        success: function (res) {
                            if (res) {
                                ...
                                $("#deptForUserDeptTree")[0]['u-meta'].tree.expandAll(true);
                                window.refmd = u.dialog({
                                    id: 'commonShowDialog',
                                    content: '#tree-deptForUserDept',
                                    hasCloseMenu: true
                                });
                                viewModel.event.bindClickButton($('#confirm_select_DeptToUserDept'), null, viewModel.event.confirmSelectDeptToUserDept);
                            } else {
                                u.showMessage({
                ...
                /**选中某一个参照到主表 */
                confirmSelectDeptToUserDept: function () {
                    var zTree = $("#deptForUserDeptTree")[0]['u-meta'].tree;
                    var selectNode = zTree.getSelectedNodes();
                    if (selectNode) {
                        var id = zTree.getSelectedNodes()[0].id;
                        var node = viewModel.refDeptDa.getRowByField('pk_dept', id).getSimpleData();
                        viewModel.dtnew.setValue('pk_dept', node.pk_dept)
                        viewModel.dtnew.setValue('pk_dept_name', node.name);
                    }
